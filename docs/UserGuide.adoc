...
=======
=== Getting auto-completed command words
Receives a suggestion of the most similar correct command word while typing in users' command. Press the `TAB` key while typing a command halfway to get a suggestion on the most possible command without having to navigate to the help window.

If there are two commands that are equally similar to hat has typed in by the user, the system will automatically fill in one of them.

****
* The input is case sensitive.
* If there are a few mistakes in what is typed by the user, press `TAB` will automatically correct the command for users.
* However if the user type in some random alphabets, and press `TAB`, the system will not auto complete the commands for users.
****

Examples:

* While typing `his` to get `history` command, press `TAB`. +
The `history` command will automatically be filled in.
* While typing `ez`, press `TAB`. +
The `edit` command will automatically be filled in even if `exit` command is equally similar to `ez`.
* While typing `hosdfqwehfjnihef`, and press `TAB`. +
The system will not auto-complete a command for the user.
>>>>>>> 27e5fbe5b20b9a98b3042ecd6a40b31abbadacc9
