...
The correspond time duration for those tasks will be deducted from the Daily Time Planner and reflected on the day list
on the right of the app.
...
If you have made a mistake on the commands, you can always undo that mistake and restores the Tasketch to the state
before the previous undoable command was executed. +
...
Result: +
You should see the state of Tasketch before the last command that you have executed.

=== Redoing the previously undone command : `redo`/`r`

If you want to go back to the state before you last undo the Tasketch, you can reverse to the most recent `undo` command. +
...
* If you want to type `list`, you can type `li` instead, and press `tap` on keyboard. The system will automatically fulfill the command in command line with `list`.
