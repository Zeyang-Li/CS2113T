...
. Tasketch provides a default account whose username and password are both "admin"
...
Result: +
The loginstatus should be changed to Logged in as the account you have logged in. 
...
Result: +
You should see the account has been added to the accountlist if you execute `listAccounts` command. 
